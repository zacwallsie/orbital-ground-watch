...
class EncoderTransformer(nn.Module):
    def __init__(
        ...
        self.block1 = nn.ModuleList(
            [EncoderBlock(dim=embed_dims[0], dim_head=4) for i in range(depths[0])]
        )
        ...
        self.block2 = nn.ModuleList(
            [EncoderBlock(dim=embed_dims[1], dim_head=4) for i in range(depths[1])]
        )
        ...
        self.block3 = nn.ModuleList(
            [EncoderBlock(dim=embed_dims[2], dim_head=8) for i in range(depths[2])]
        )
        ...
        self.block4 = nn.ModuleList(
            [EncoderBlock(dim=embed_dims[3], dim_head=8) for i in range(depths[3])]
        )
...
class ScratchFormer(nn.Module):
    def __init__(self, input_nc=6, output_nc=1, decoder_softmax=False, embed_dim=512):
